...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      const mod = await import('@hcengineering/recruit-resources/src/components/Applications.svelte')
      Applications = mod.default
    } catch {
      notifyError('Failed to load applications component')
    }
    loading = false
  })
...
<div class="embed-applications" bind:this={container}>
  {#if config.space === undefined}
    <div class="embed-error">Missing space parameter (candidate ID)</div>
  {:else if loading}
    <div class="embed-loading">Loading...</div>
  {:else if Applications !== undefined}
    <svelte:component
      this={Applications}
      objectId={config.space}
      applications={0}
