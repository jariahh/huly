...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      const mod = await import('@hcengineering/chunter-resources/src/components/threads/ThreadView.svelte')
      ThreadView = mod.default
    } catch {
      notifyError('Failed to load thread component')
    }
    loading = false
  })
...
<div class="embed-thread" bind:this={container}>
  {#if config.thread === undefined}
    <div class="embed-error">Missing thread parameter</div>
  {:else if loading}
    <div class="embed-loading">Loading...</div>
  {:else if ThreadView !== undefined}
    <svelte:component
      this={ThreadView}
      _id={config.thread}
      showHeader={false}
