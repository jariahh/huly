...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      const mod = await import('@hcengineering/tracker-resources/src/components/issues/timereport/TimeSpendReports.svelte')
      TimeSpendReports = mod.default
    } catch {
      notifyError('Failed to load TimeSpendReports component')
    }
    loading = false
  })
...
<div class="embed-time-reports" bind:this={container}>
  {#if config.issue === undefined}
    <div class="embed-error">Missing issue parameter</div>
  {:else if loading}
    <div class="embed-loading">Loading...</div>
  {:else if issueDoc !== undefined && TimeSpendReports !== undefined}
    <svelte:component
      this={TimeSpendReports}
      issue={issueDoc}
      query={{ attachedTo: issueDoc._id }}
    />
