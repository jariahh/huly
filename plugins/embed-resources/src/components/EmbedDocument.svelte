...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      const mod = await import('@hcengineering/document-resources/src/components/EditDoc.svelte')
      TargetComponent = mod.default
    } catch {
      notifyError('Failed to load document component')
    }
    loading = false
  })
...
<div class="embed-document" bind:this={container}>
  {#if config.document === undefined}
    <div class="embed-error">Missing document parameter</div>
  {:else if loading}
