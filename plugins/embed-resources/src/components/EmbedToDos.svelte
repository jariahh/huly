...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      const mod = await import('@hcengineering/time-resources/src/components/ToDos.svelte')
      ToDos = mod.default
    } catch {
      notifyError('Failed to load todos component')
    }
    loading = false
  })
...
<div class="embed-todos" bind:this={container}>
  {#if loading}
    <div class="embed-loading">Loading...</div>
  {:else if ToDos !== undefined}
    <svelte:component
      this={ToDos}
      mode={config.mode ?? 'planned'}
      tag={undefined}
