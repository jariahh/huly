...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      const mod = await import('@hcengineering/drive-resources/src/components/FolderBrowser.svelte')
      TargetComponent = mod.default
    } catch {
      notifyError('Failed to load file browser component')
    }
    loading = false
  })
...
<div class="embed-file-browser" bind:this={container}>
  {#if config.drive === undefined}
    <div class="embed-error">Missing drive parameter</div>
  {:else if loading}
  ...
  {:else if TargetComponent !== undefined}
    <svelte:component
      ...
      space={config.drive}
      parent={config.folder ?? config.drive}
      type={'drive'}
