...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      const mod = await import('@hcengineering/lead-resources/src/components/MyLeads.svelte')
      MyLeads = mod.default
    } catch {
      notifyError('Failed to load leads component')
    }
    loading = false
  })
...
<div class="embed-my-leads" bind:this={container}>
  {#if loading}
    <div class="embed-loading">Loading...</div>
  {:else if MyLeads !== undefined}
    <svelte:component
      this={MyLeads}
    />
  {/if}
